...
public class DroneNavigationAI : MonoBehaviour {
    ...
    void ShootTarget(GameObject target) {
        RaycastHit[] hits;
        bool flyByTrue = true;
        Vector3 DroneBodyOffset = new Vector3(0f, 0f, 0.7f);
        int decalHitCount = 0;//to know how many walls we've hit limit bullet decal spray
        hits = RaycastAllNonConvex(transform.position + DroneBodyOffset, (target.transform.position - (transform.position + DroneBodyOffset)).normalized);
        // Physics.RaycastAll(transform.position, transform.forward);//.OrderBy(h=>h.distance).ToArray();

        //Debug.Log("Origin: " + transform.position + ", direction: " + target.transform.position);
        Debug.DrawRay(transform.position + DroneBodyOffset, (target.transform.position - transform.position + DroneBodyOffset).normalized, Color.red);
